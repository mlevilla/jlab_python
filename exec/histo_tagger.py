...
# argument parsing
largs = [('r',982,'several','run_number'),
         ('i','','','input_folder'),
...
[run,indir,method,outdir,write,path,new_offsets] = aparser.argl

# batch mode
if batch[0]:
  runs = get_runs_between(run[0],run[-1])
  jsub_fast(aparser,'histo_tagger',['r','o'],[['r'],runs],outdir,jobname='tagger',time=wtime[0])
  sys.exit()

run = str(run[0]) if len(run)==1 else str(run[0])+'_'+str(run[-1])
...
for _ in progress(t,show=show_progress[0],n=t.GetEntries(),precision=1):
  ...
  idg2,tg2,idg3 = [],[],[]
  if t.trigger not in [1,2,6]: continue
