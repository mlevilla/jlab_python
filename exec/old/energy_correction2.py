...
if correct: 
 cpar_ene = readfile(correct,[float]*5,cols=range(1,6))
 lpar_ene = len(cpar_ene)/2

def correct_energy(tx,ty,ie,iregion):
  i = lpar_ene*ie+iregion
  return 1./cpar_ene[i][0]/(1+cpar_ene[i][1]*tx**2+cpar_ene[i][2]*tx**4)/(1+cpar_ene[i][3]*ty**2+cpar_ene[i][4]*ty**4)
...
helas = listofhisto('helas',[range(nindex),range(2)],1000,-1,1,title=';E\' (MeV);')
...
if correct:
  ...
  hyEc = listofhisto('hyEc',[range(nindex),range(2)],nbin,-1.,1.,title=';(y_{hycal}-y_{center})/d;')
...
for _ in progress(t,show=show_progress[0],n=t.GetEntries(),precision=1):
  ...
  for j in range(len(idx)):
    ...
    _ = hx[idx_module][0].Fill(new_tx)
    _ = hy[idx_module][0].Fill(new_ty)
    ...
    _ = hxE[idx_module][0].Fill(new_tx,elas_ep+1)
    _ = hyE[idx_module][0].Fill(new_ty,elas_ep+1)
    ...
    if correct:
      ...
      _ = hxEc[idx_module][0].Fill(new_tx,elas_ep+1)
      _ = hyEc[idx_module][0].Fill(new_ty,elas_ep+1)
  ...
  for j in range(2):
    ...
    _ = hx[idx_module][1].Fill(new_tx)
    _ = hy[idx_module][1].Fill(new_ty)
    ...
    _ = hxE[idx_module][1].Fill(new_tx,elas_m+1)
    _ = hyE[idx_module][1].Fill(new_ty,elas_m+1)
    ...
    if correct:
      ...
      _ = hxEc[idx_module][1].Fill(new_tx,elas_m+1)
      _ = hyEc[idx_module][1].Fill(new_ty,elas_m+1)
