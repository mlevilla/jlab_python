...
class ArgParser:
  ...
  def __init__(self,largs=[],argv=[],empty=1,lmax=15):
    self.largs = largs
    self.argv = []
    self.argd = Mdict()
    self.argl = []
  ...
  def catch(self,mark,default,opt=''):
    ...
    if '-'+mark in self.argv:
      ...
      if opt=='':
        r = [not default[0]] if isinstance(default[0],bool) else [type(default[j])(self.argv[i+j+1]) for j in range(len(default)) if i+j+1<len(self.argv)]
      elif opt=='several':
        k = i+1
  ...
  def print_help(self,empty=1,lmax=15):
    if '-h' in self.argv or '--help' in self.argv or (empty and len(self.argv)==1):
