...
def progress(l,modulo=100,precision=0,show=2,width=30,m='<=>',n=-1,limit=1.,insert=None):
  ...
  for i,x in enumerate(l):
    ...
    if (n==-1 and i%modulo==0) or (n!=-1 and i>=count*n/float(modulo)):
      ...
      count+=1
      if (isinstance(limit,int) and i>limit) or (n!=-1 and isinstance(limit,float) and count/float(modulo)>limit): break
    # insert
    if insert!=None:
      ins = str(insert(x))
      n_ins = len(ins)+2
      st = st[:width/2-n_ins/2]+ins.center(n_ins)+st[width/2+n_ins/2+1:]
      write_and_flush(st)
  # end of loop
  if n==-1: pass
...
def print_help(l,empty=1,lmax=15):
  if '-h' in sys.argv or '--help' in sys.argv or (empty and len(sys.argv)==1):
    print 'command:',sys.argv[0]
    print 'inputs:'
    max_length0 = max(len(x[0]) for x in l)
    max_length1 = max(min(len(str(x[1])),lmax+3)+len(x[3]) for x in l)
    for x,y,z,c in l:
...
def catch_args(l):
  ...
  return [catch_arg(*x[:-1],args=args) for x in l]
...
def unnest(l):
  ...
  return l2  
#sys.stdout.write('misc.py loaded in '+str(round(time.time()-time_start,2))+' seconds\n')
